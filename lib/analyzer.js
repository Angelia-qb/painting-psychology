...
import { SHARE_QUOTE_SYSTEM, buildShareQuotePrompt } from './sharePrompt.js';
 ...
 * 生成分享卡片金句（纯文本，不含图片）
 */
export async function generateShareQuote({ drawingTitle, report }) {
  const provider = (process.env.AI_PROVIDER || 'openai').toLowerCase();
  if (provider === 'mock') return '这是开发模式下的占位金句';

  const userPrompt = buildShareQuotePrompt({ drawingTitle, report });
  ...
  const callText = async () => {
    if (provider === 'anthropic') {
      ...
      const res = await fetch(`${baseUrl}/v1/messages`, {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 200,
          temperature: 1.0,
          system: SHARE_QUOTE_SYSTEM,
          messages: [{ role: 'user', content: userPrompt }]
        })
      });
      if (!res.ok) throw makeApiError('Anthropic', res, await res.text());
    ...
    const res = await fetch(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        temperature: 1.0,
        messages: [
          { role: 'system', content: SHARE_QUOTE_SYSTEM },
          { role: 'user', content: userPrompt }
        ]
      })
    });
    if (!res.ok) throw makeApiError('OpenAI', res, await res.text());
    const d = await res.json();
  ...
  const quote = clean(raw.match(/QUOTE:\s*(.+)/)?.[1]);
  const question = clean(raw.match(/QUESTION:\s*(.+)/)?.[1]);

  // 模型没按格式输出时，退化为整段当金句
  if (!quote) return { quote: clean(raw.split('\n')[0]), question: '' };
