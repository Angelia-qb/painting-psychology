...
export function createInviteStore(dataDir) {
  ...
  return {
     ...
     * 同一天重复调用返回已有的，不会重复生成。
     */
    dailyCodesFor(userId, username, perDay = 3) {
      const all = readAll();
      const today = new Date(Date.now() + 8 * 3600_000).toISOString().slice(0, 10);

      const mine = Object.values(all).filter(
        (i) => i.ownerId === userId && i.issuedDay === today
      );
      if (mine.length >= perDay) return mine;

      for (let n = mine.length; n < perDay; n++) {
        let code;
        do {
          code = randomCode();
        } while (all[code]);
    ...
    /** 某个码的持有者（用于发放邀请奖励） */
    ownerOf(code) {
      const inv = readAll()[normalizeInvite(code) || ''];
      return inv?.ownerId || null;
    },

    /** 用户累计成功邀请人数 */
    inviteStats(userId) {
      const list = Object.values(readAll()).filter((i) => i.ownerId === userId);
      return {
        issued: list.length,
        used: list.reduce((s, i) => s + i.usedCount, 0),
        invitees: list.flatMap((i) => i.usedBy.map((u) => u.username))
      };
    },
